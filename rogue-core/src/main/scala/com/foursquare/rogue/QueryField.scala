...
class ObjectIdQueryField[F <: ObjectId, M](override val field: Field[F, M])
  ...
  def before(d: DateTime) =
    new LtQueryClause(field.name, new ObjectId(d.toDate, 0))

  def after(d: DateTime) =
    new GtQueryClause(field.name, new ObjectId(d.toDate, 0))

  def between(d1: DateTime, d2: DateTime) =
    new StrictBetweenQueryClause(field.name, new ObjectId(d1.toDate, 0), new ObjectId(d2.toDate, 0))

  def between(range: (DateTime, DateTime)) =
    new StrictBetweenQueryClause(field.name, new ObjectId(range._1.toDate, 0), new ObjectId(range._2.toDate, 0))
}
