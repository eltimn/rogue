...
trait LiftRogue {
  ...
  implicit def bsonRecordListFieldToBsonRecordListQueryField[
    ...
    val rec = f.setFromJValue(JArray(JInt(0) :: Nil)).openOrThrowException("a gross hack to get at the embedded record").head // a gross hack to get at the embedded record
